...
alter table public.campuses add column if not exists created_at timestamptz not null default now();
-- Add unique constraint on slug if not present
do $$ begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'campuses_slug_key' and conrelid = 'public.campuses'::regclass
  ) then
    alter table public.campuses add constraint campuses_slug_key unique (slug);
  end if;
exception when others then null;
end $$;

-- Set slug for existing rows that have null slug
update public.campuses set slug = lower(regexp_replace(name, '[^a-z0-9]+', '-', 'gi')) where slug is null;
...
-- =========================================================
-- DEMO / DIRECT-LOGIN USER PROFILE
-- Matches defaultLoginId in App.js (00000000-0000-4000-a000-000000000002)
-- This ensures the demo login works without real Supabase auth.
-- =========================================================
...
-- Safely add any missing columns to existing profiles table
...
alter table public.profiles add column if not exists open_to_projects boolean default false;
...
-- Seed demo user profile (matches App.js defaultLoginId = 00000000-0000-4000-a000-000000000002)
do $$
declare
  v_campus_id uuid;
  v_existing_id uuid;
begin
  -- Get the NHCE campus id
  ...
  where email = 'sanjaypadmaraj@nhce.edu.in' limit 1;

  if v_existing_id is not null then
    -- Email exists: just update the fields (don't change the PK to avoid FK issues)
    update public.profiles set
      ...
      year = '2nd Year',
      role = 'student'
    where email = 'sanjaypadmaraj@nhce.edu.in';
  else
    -- No row with this email â€” insert with the hardcoded UUID
...
cross join (
  values
    ...
    ('Tango','Rolls Ã‚Â· Noodles Ã‚Â· Biryani Ã‚Â· Pasta','Moderate',12,18,'moderate',58,'moderate'),
    ('Munch','Fried Rice Ã‚Â· Noodles Ã‚Â· Chinese','Busy',20,28,'busy',84,'busy'),
    ('Nescafe','Coffee Ã‚Â· Maggi Ã‚Â· Snacks','Quiet',6,10,'quiet',26,'green')
...
drop policy if exists "canteens read" on public.canteens;
drop policy if exists "food categories read" on public.food_categories;
drop policy if exists "food items read" on public.food_items;
...
drop policy if exists "club members insert own" on public.club_members;
drop policy if exists "club members delete own" on public.club_members;
drop policy if exists "events read" on public.events;
drop policy if exists "event registrations read own" on public.event_registrations;
drop policy if exists "event registrations insert own" on public.event_registrations;
...
drop policy if exists "lost found update own" on public.lost_found_items;
drop policy if exists "marketplace read" on public.marketplace_listings;
drop policy if exists "marketplace insert own" on public.marketplace_listings;
drop policy if exists "marketplace update own" on public.marketplace_listings;
...
drop policy if exists "service requests own" on public.service_requests;
...
drop policy if exists "print jobs own" on public.print_jobs;
drop policy if exists "print jobs insert own" on public.print_jobs;
...
-- Enable Realtime for all interactive tables (safe: ignore if already added)
do $$ begin
